...
def load_models(feature_method: str):
    ...
    if TF_AVAILABLE:
        for name in ["LSTM", "BiLSTM", "GRU"]:
            path = os.path.join(base_dir, f"{name}.h5")
...
def render_model_breakdown(model_results):
    if not model_results:
        return
    st.markdown("<br>", unsafe_allow_html=True)
    section("📈 Individual Model Predictions")
    mcols = st.columns(len(model_results))
    for col, (name, res) in zip(mcols, model_results.items()):
        with col:
            st.metric(
                label=name,
...
with tab1:
    ...
    if st.button("🔬 Run Outbreak Prediction", key="btn_manual"):
        ...
        if final_pred is None:
            st.warning(f"⚠️ No trained models found at `saved_models/{feature_method}/`. Train and save your models first.")
        else:
            render_result(final_pred, final_prob)
            render_model_breakdown(model_results)
